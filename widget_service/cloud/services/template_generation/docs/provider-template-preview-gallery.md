...
数据集覆盖 121 个业务模板，不包含 6 个布局模板：

- Support：32 个，使用 2×2 卡片容器，模板内容高度 68vp；原子预览省略可选内部事件。
- Compact：32 个，使用 2×2 卡片容器，模板内容高度 68vp。
- Hero：20 个，使用 2×2 卡片容器，模板内容高度 124vp。
...
- `T001.json` 至 `T121.json`：每个模板独立的 A2UI 消息数组，固定包含 `createSurface`、
