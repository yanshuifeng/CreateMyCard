...
数据集覆盖 91 个业务模板，不包含 6 个布局模板和 2 个 Action 模板：

- Support：19 个，使用 2×2 卡片容器，模板内容高度 68vp；原子预览省略可选内部事件。
- Compact：20 个，使用 2×2 卡片容器，模板内容高度 68vp。
- Hero：19 个，使用 2×2 卡片容器，模板内容高度 124vp。
- Full：20 个，使用 2×2 卡片容器，模板内容高度 136vp。
...
- `T001.json` 至 `T091.json`：每个模板独立的 A2UI 消息数组，固定包含 `createSurface`、
