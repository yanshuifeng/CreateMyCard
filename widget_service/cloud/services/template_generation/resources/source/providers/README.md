...
- `layout`：无数据能力 → 17 个支持 `...children` 的布局模板；仅含 `Wide` 的布局用于 `2x4`
- `action`：无数据能力 → `PillAction@1`、`IconAction@1`、`LargeIconAction@1` 三个 Props 驱动的动作模板
...
角色校验。2x2 Action 使用第一层独立选择的零到两个 `eventId`；2x4 兼容布局保留四动作槽等既有契约。
第二层按业务模板后缀调用布局末尾的 `PillAction@1`、`IconAction@1` 或 `LargeIconAction@1` 并填写 Props；
Action Provider 拥有组件结构，微服务只注入主题色和
